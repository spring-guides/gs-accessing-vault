...
:install-link:https://developer.hashicorp.com/vault/tutorials/get-started/install-binary

With your project set up, you can {install-link}[install] and launch HashiCorp Vault.
...
For other systems with package management, such as Redhat, Ubuntu, Debian, CentOS, and Windows, see the {install-link}[official instructions].
...
As our `Application` exposes an `ApplicationRunner` bean, it is invoked automatically when Spring Boot starts.
